...
source "$thisDir/scripts/.constants.sh" \
	...
	'./examples/build-inmage.sh output stretch 2017-05-08T00:00:00Z
--no-build --image=geosupport-docker:latest ./examples/ubuntu.sh output xenial'
...
if [ -n "$build" ]; then
	echo docker build ${pull:+--pull} --tag "$image" "$thisDir"
elif [ -n "$pull" ]; then
	echo docker pull "$image"
else
	# make sure "docker run" doesn't pull (we have `--no-build` and no explicit `--pull`)
	echo docker image inspect "$image" > /dev/null
fi
...
echo exec docker run "${args[@]}" "$image" "$@"
