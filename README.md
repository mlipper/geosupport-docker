...
* **Version 2.0.0 available.** This release wraps `Geosupport 22a2_22.11`.
  ...
  **CHANGES:**
  ...
  >   * Use templates to generate artifacts in versioned directories which are commited to git
  >   * Automate project build and release tasks with `<project_dir>/release.sh`.
  >   * Externalize project build-time configuration in `<project_dir>/release.conf`.
  >   * Create `<project_dir>/templates` for better sharing of common build/run logic.
  >   * Reduce excessive use of environment variables favoring build-time configuration (see <project_dir>/release.conf`)
  >   * Inspired by the [Docker Official Images](https://github.com/docker-library/official-images) repo's use of [bashbrew](https://github.com/docker-library/bashbrew). 
  ...
  >   * New default value: `${GEOSUPPORT_BASE}/current`
  >   * `current` directory is a symlink (helpful for certain volume and bind mount scenarios to allow multiple Geosupport installations with quit version switching).
...
Coming soon: `v2.0.1` for Geosupport `22a2_22.12`.
